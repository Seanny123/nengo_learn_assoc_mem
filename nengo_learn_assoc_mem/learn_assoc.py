...
class LearningAssocMem(nengo.Network):
    def __init__(self, n_neurons, dimensions,
        ...
        if load_from is not None:
            if type(load_from) is dict:
                encoders = load_from['enc']
                decoders = load_from['dec']

                if seed is None:
                    seed = load_from['seed']

            elif type(load_from) is str:
                ...
                raise TypeError("Can only load from numpy or dict")
        else:
        ...
        with self:
            ...
            self.correct = nengo.Node(size_in=dimensions)
            ...
            if voja_rate > 0 or pes_rate > 0:
                self.stop_learn = nengo.Node(size_in=1)

            if voja_rate > 0:
                nengo.Connection(self.stop_learn, self.conn_in.learning_rule,
                                 synapse=None)
            ...
            if pes_rate > 0:
                ...
                # Control connection
                nengo.Connection(self.stop_learn, self.learn_control[-1],
                                 synapse=None)

            if inhibit_all:
                inhibit = nengo.Node(size_in=1)
